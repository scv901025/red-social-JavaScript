const urlBase='https://jsonplaceholder.typicode.com/posts'//esta es la URL con la que se interactua
let posts = [] //iniciamos los posteos como un array vacio

function getData(){
    fetch(urlBase)  //hace llamado a los post(base de datos) en este caso API falsa
    .then(res => res.json()) //de encontrar datos los convierte en un json
    .then(data => {
        posts=data
        renderPostList()
    })
...
function postData(){
    ...
    fetch(urlBase, {
      ...
      .catch (error => console.error('error del query al crear el post: ', error))
}
...
function updatePost(id){
    ...
    .catch (error => console.error('error al querer actualizar: ', error))
}
